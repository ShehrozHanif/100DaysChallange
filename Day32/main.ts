...
let word:string[] = ["Hello","World","TypeScript","JavaScript"]
const lengths:number[] = word.map(word => word.length)
console.log(lengths)
...
function calculateNumber(num3:number[]):number{
   
    let lol = num3.reduce((total,sum)=>total + sum, 0)
    return lol
}

const num3:number[] =[5,10,25,15,5] 
